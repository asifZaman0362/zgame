...
class LightingSystem : public ISystem {
   public:
    Signature GetSignature() override {
        return zifmann::zgame::core::utils::SignatureHelper(
            1, GetComponentId<Light>());
    }
    void Update(float) override {
        ...
        for (auto light : m_entities) {
            ...
            // auto pos =
            // coordinator.GetComponent<zifmann::zgame::core::Position
            /*for (auto material : shaders) {
                auto lightPos = glGetUniformLocation(material.shader,
            "lightPos"); auto [x, y, z] = light_source.; glUniform3f(lightPos,
            x, y, z); auto lightCol = glGetUniformLocation(mesh->shader,
            "lightColor"); auto [r, g, b] = light_source.color;
